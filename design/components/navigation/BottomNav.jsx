...
function NavIcon({ type, color }) {
  ...
  if (type === 'list')
    return (
      <svg {...common}>
        <rect x="3" y="5" width="16" height="2.4" rx="1.2" fill={color} />
        <rect x="3" y="10" width="16" height="2.4" rx="1.2" fill={color} />
        <rect x="3" y="15" width="10" height="2.4" rx="1.2" fill={color} />
      </svg>
    );
  if (type === 'target')
    return (
  ...
  if (type === 'profile')
    return (
      <svg {...common}>
        ...
        <path
          d="M3.5 19c1.3-4 4.3-6 7.5-6s6.2 2 7.5 6"
          stroke={color}
          strokeWidth="1.8"
          strokeLinecap="round"
        />
...
function NavItem({ item, active, onChange }) {
  ...
  return (
    <div
      ...
      <span style={{ fontSize: 10, fontWeight: 'var(--weight-bold)', color: textColor }}>
        {item.label}
      </span>
    </div>
  );
  ...
  return (
    <div
      ...
      {items.slice(0, 2).map((it) => (
        <NavItem key={it.key} item={it} active={it.key === activeKey} onChange={onChange} />
      ))}
      <div
        onClick={onAdd}
        style={{
